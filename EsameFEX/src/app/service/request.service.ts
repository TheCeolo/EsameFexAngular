...
//ho creato l'oggetto qua per semplificare l'avvio successivo del progetto

  constructor() { }

  getAllPlace() {
    return this.listLocalita
  }
  getPlace(id_place: number) {
    return this.listLocalita[id_place - 1]
  }
  getAttrazioni(id_place: number) {
    return this.listLocalita[id_place].attrazioni
  }
  getAttrazionibyId(id_place: number, id_attr: number) {
    return this.listLocalita[id_place - 1].attrazioni[id_attr - 1]
  }
